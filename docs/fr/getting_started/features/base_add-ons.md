# Modules complémentaires de base 
 Ces fonctionnalités étendent les capacités décrites dans Base, servant soit à améliorer l’exhaustivité d’un ensemble de données GTFS pour offrir une meilleure expérience aux passagers, soit à faciliter la collaboration entre les agences, les fournisseurs de données et les réutilisateurs de données. Ces améliorations peut impliquer l’introduction de nouveaux champs dans les fichiers décrits dans Base ou la création de nouveaux fichiers. 
...
## Formes 
 Des formes peuvent être définies et associées à des trajets, permettant aux applications de planification de trajets d’afficher les trajets sur une carte et d’informer les usagers de la distance qu’ils doivent parcourir dans un vehicle de transport en commun. Les champs `shape_dist_traveled` sont utilisés pour déterminer par programme la quantité de shape à dessiner lors de l’affichage d’une carte aux passagers. 
 Lors de la définition des formes, il existe un équilibre entre leur niveau de détail (par exemple suivre la courbure exacte des routes) et la transmission efficace uniquement des informations nécessaires. 
...
<p style="font-size:16px"> 
 L’exemple suivant montre une partie d’une shape du flux TriMet GTFS (téléchargez-le <a     href="https://developer.trimet.org/GTFS.shtml">ici</a> ).<br><br> 
</p> 
 !!! note "" 
<p style="font-size:16px"> 
...
## Couleurs des itinéraires 
 ...
 L’utilisation des couleurs des itinéraires permet de représenter et de communiquer avec précision la palette de couleurs attribuée à des routes spécifiques par les directives de conception de l’agence. Cela permet aux utilisateurs d’identifier facilement les services de transport en commun par leur couleur officielle. 
...
<p style="font-size:16px"> 
 L’exemple suivant montre que la route « RA » est orange en utilisant un code de couleur HEX « D95700 » et que le texte devrait être rendu en noir en utilisant un code de couleur HEX « 0 ». 
</p> 
 !!! note "" 
...
## Bike Allowed 
 ...
 Bike Allowed indique si les véhicules effectuant des déplacements spécifiques sont en mesure d’accueillir des vélos ou non, aidant ainsi les utilisateurs à planifier et à accéder aux services qui leur permettent d’effectuer des déplacements multimodaux. 
...
<p style="font-size:16px"> 
 L’exemple suivant précise que le vehicle utilisé dans le voyage `AWE1` peut accueillir au moins un vélo à bord (`bikes_allowed=1`), et que le vehicle utilisé dans le voyage `AWE2` ne le peut pas (`bikes_allowed=2`). 
</p> 
 !!! note "" 
<p style="font-size:16px"> 
...
## Headsigns 
 ...
 Headsigns permet de communiquer la signalisation utilisée par les véhicules indiquant la destination du voyage, permettant ainsi aux utilisateurs d’identifier plus facilement le bon service de transport en commun. Cette fonctionnalité prend en charge les changements de phare le long d’un itinéraire spécifique. 
...
## Service basé sur la fréquence 
 ...
 Lors de la modélisation d’un service basé sur la fréquence, `stop_times.txt` contient les temps relatifs entre les stops afin de déterminer les temps à afficher aux passagers. 
...
## Transferts 
 ...
 Les transferts fournissent des détails sur les transitions entre différents segments (ou étapes) de voyage, permettant aux planificateurs de voyage de déterminer la faisabilité des voyages incluant des transfers. Spécifier les transfers n’implique pas que les passagers ne peuvent t effectuer de transfert ailleurs, cela indique simplement si certains transfers ne sont pas possibles ou require un temps minimum de transfert. 
...
<p style="font-size:16px"> 
 L’exemple suivant montre trois transfers différents : un entre les stops qui nécessite un temps de transfert minimum de 5 minutes, un point de transfert chronométré entre deux routes et un transfert au siège entre deux trajets effectués par le même vehicle. 
</p> 
 !!! note "" 
<p style="font-size:16px"> 
...
## Traductions 
 ...
 | Fichiers inclus | Champs inclus | 
 |------------------------|-------------------| 
 |[translations.txt](../../../documentation/schedule/reference/#translationstxt)|`table_name`,`field_name`,`langue`,`translation`,`record_id`,`record_sub_id`,`field_value` | 
...
<p style="font-size:16px"> 
 ...
 | table_name | field_name | langue | Traduction | record_id | record_sub_id | field_value | 
 |------------|-----------------|----------|-----------------------------------------------------------|---------------|---------------|-------------| 
 | routes | route_long_name | ES | Mission- Centre | RA | | | 
 | routes | route_long_name | FR | Mission- Centre ville | RA | | | 
 | routes | route_desc | ES | La route "A" via Lower Mission jusqu’au centre | RA | | | 
