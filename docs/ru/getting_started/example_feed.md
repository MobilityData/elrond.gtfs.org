# Пример фида GTFS 
 ...
 В этом примере фида GTFS показаны образцы данных, разделенные запятыми, для каждого файла в транзитном фиде. Показанные здесь примеры файлов данных t все связаны друг с другом. Вы также можете [загрузить полный фид GTFS](https:) в окончательной форме.для работы или просмотрите отдельные файлы этого же канала в Интернете [здесь](https://github.com/google/transit/tree/master/gtfs/spec/en/examples/sample-feed-1). 
...
## agency.txt 
 ...
 агентство001,Транзитное агентство,https://www.transitcommuterbus.com/, PST,en 
 [...] 
 ``` 
...
## routes.txt 
 ...
 A,17,Mission,"Маршрут "A" проходит из нижней Миссии в центр города. ",3 
 ``` 
...
## calendar_dates.txt 
 ...
 Этот пример показывает исключения обслуживания для праздника Дня независимости в 2006 году. В понедельник, 3 июля 2006 года, обычное обслуживание в будние дни (`service_id=WD) прерывается (`exception_type=2 ). Вместо этого в этот date запускается служба выходного дня (` service_id=WE ) (` exception_type=1). То же изменение произойдет и во вторник, 4 июля. 
...
## levels.txt 
 ...
 ``` 
 level_id, level_index, level_name,высота 
 L0,0,Street,0 
 L1,-1,Мезонин,-6 
 L2,-2,Южное направление,-18 
 L3,-3,Северное направление,- 24 
 ``` 
 
## translations.txt 
 
 ``` 
 table_name, field_name,язык,перевод, record_id 
