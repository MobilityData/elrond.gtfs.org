...
### trips.txt 
 ...
 | `block_id| identificación | Opcional | Identifica el bloque al que pertenece el viaje. Un bloque consta de un único viaje o de muchos viajes secuenciales realizados utilizando el mismo vehicle, definidos por días de servicio compartido y `block_id. Un `block_id puede tener viajes con diferentes días de servicio, formando bloques distintos. Vea el [ejemplo a continuación](#ejemplo-bloques-y-día-de-servicio). Para proporcionar información transfers en el asiento, se deben proporcionar [transfers](#transferstxt) de `transfer_type`4`. | 
...
### transfers.txt 
 ...
 | `to_stop_id` | Identificación extranjera que hace referencia a `stops.stop_id| **Condicionalmente Obligatorio** | Identifica una parada o estación donde termina una conexión entre routes. Si este campo se refiere a una estación, la regla de transferencia se aplica a todas las stops secundarias. Hacer referencia a una estación está prohibido para `transfer_types` 4 y 5. | 
...
: este archivo utiliza un subconjunto del formato GeoJSON, descrito en [RFC 7946](https://tools.ietf.org/html/rfc7946). 
 ...
 |         \- `tipo` | Cadena | **Obligatorio** | Debe ser del tipo:<br> - `"Polygon"`<br> - `"MultiPolígono"` | 
