# GTFS 피드 예시 
 ...
 이 GTFS 피드 예시는 대중교통 피드의 각 파일에 대한 쉼표로 구분된 데이터 샘플을 보여줍니다. 여기에 표시된 샘플 데이터 파일은 모두 서로 관련되어 있지 t. 최종 형식으로 [전체 GTFS 피드를 다운로드](https://github.com/google/transit/blob/master/gtfs/spec/en/examples/sample-feed-1.zip?raw=true)할 수도 있습니다. 작업하거나 온라인으로 동일한 피드의 개별 파일을 찾아보세요. [여기](https://github.com/google/transit/tree/master/gtfs/spec/en/examples/sample-feed-1). 
...
## agency.txt 
 ...
 기관001,교통 기관,https://www.transitcommuterbus.com/, PST,en 
 [...] 
 ``` 
...
## routes.txt 
 ...
 A,17,Mission,"""A"" 경로는 하위 미션에서 다운타운으로 이동합니다. ",3 
 ``` 
...
## calendar_dates.txt 
 ...
 이 예 2006년 독립기념일 휴일에 대한 서비스 예외를 보여줍니다. 2006년 7월 3일 월요일에 정규 평일 서비스(`service_id=WD`)가 중단됩니다(`exception_type=2 `). 대신 해당 date (`exception_type=1 )에 주말 서비스(` service_id=WE )가 실행됩니다. 7월 4일 화요일에도 동일한 변경 사항이 적용됩니다. 
...
## levels.txt 
 ...
 ``` 
 level_id, level_index, level_name, 고도 
 L0,0,Street,0 
 L1,-1,Mezzanine,-6 
 L2,-2,Southbound,-18 
 L3,-3,Northbound,- 24 
